...
const checkBusinessRuleOneOfeachMenu = async (user, menuValue) => {
  let isBusinessRuleToBeBroken = false
  if (menuValue === 'true') {
    try {
      const menuRestaurants = await Order.findByPk({ where: { userId: user, menu: true }, include: [{ model: Product, include: [{ model: ProductCategory }] }] })
      if (menuRestaurants.length !== 0) {
        const categories = ['bebida', '1 plato', '2 plato', 'postre']
        const categoryCounts = {}
        for (let i = 0; i < categories.length; i++) {
          categoryCounts[categories[i]] = menuRestaurants.filter(restaurant => restaurant.ProductCategory.name === categories[i]).Product.length
        }
        const counts = Object.values(categoryCounts)
        isBusinessRuleToBeBroken = !counts.every(count => count === counts[0])
      }
    } catch (error) {
      isBusinessRuleToBeBroken = true
    }
  }
  return isBusinessRuleToBeBroken ? Promise.reject(new Error('Debe tener la misma cantidad de cada categoría del menú')) : Promise.resolve()
}

async function checkOrderCanBeSent (value, { req }) {
  try {
    const order = await Order.findByPk(req.params.orderId,
      {
  ...
  create: [
    check('menu')
      .custom(async (value, { req }) => {
        return checkBusinessRuleOneOfeachMenu(req.user.id, value)
      })
      .withMessage('You need to select a product of each type to complete the menu')
